        ...
        <body>
          ...
          <main>
            ...
            <div class="container-fluid d-flex justify-content-around flex-wrap w-100 py-3 px-5 gap-1">
              <% try{ String search=request.getParameter("product"); Crud c=new Crud("root",""); String
                query="SELECT * from `products`" ; if(search !=null && !search.isEmpty()){
                query+=" WHERE `name` LIKE '%" +search+"%'"; } ResultSet data=c.getData(query); while(data.next()){ %>
                ...
                <% } }catch(Exception err){ Helper.setAlert(true, err.getMessage(), request); } %>
            </div>
            <% if(session.getAttribute("isAlert") !=null){ boolean isAlert=(boolean) session.getAttribute("isAlert");
              if(isAlert){ boolean isErr=(boolean) session.getAttribute("isError"); %>
              <div class="alert <%= isErr ? " alert-danger" : "alert-success" %> alert-dismissible fade show
                position-absolute top-0 w-100" role="alert">
                <strong>
                  <%= !isErr ? "Success" : "Error" %>!
                </strong>
              ...
              <% session.removeAttribute("isError"); session.removeAttribute("message");
